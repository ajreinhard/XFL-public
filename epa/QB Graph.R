...
ggplot(data = all_qb_plays, aes(x = PasserRusher, y = epa_per_play, fill = ClubCode)) + 
	...
	#geom_image(aes(image = logos, y = logos_y), size = 0.12) +
	...
	labs(title='XFL QB EPA per Play',
       ...
       subtitle='Through Week 3 of 2020 using NFL Expected Points model',
       y = 'EPA / Play') +
	theme_bw() +
	theme(
...
ggsave('XFL QB Week 3.png', width=5 * (16/9), height=5)
