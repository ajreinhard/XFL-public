...
xfl_air_yards_bam <- bam(complete_pass ~ s(air_yards), data = pbp, method = "fREML", subset = throwaway==0, discrete = TRUE, family = binomial(link='logit'))
...
team_list <- pbp$posteam[match(player_list, pbp$passer_player_name)]
...
player_bam <- function(pl_name) {
  ...
  player_air_yards_bam <- bam(complete_pass ~ s(air_yards), data = pbp, method = "fREML", discrete = TRUE, family = binomial(link='logit'), subset = passer_player_name==pl_name & throwaway==0)
...
pred_df$true_comp_pred_XFL <- exp(pred_df$pred_XFL)/(1+exp(pred_df$pred_XFL))
...
ggplot(data = pred_df, aes(x = air_yards)) +
  ...
  labs(title = 'XFL Completion Percentage by Depth of Target', 
       ...
       caption  = 'By Anthony Reinhard  |  Data from @xflscrapR via @CFB_Moose  |  min. 75 Pass Attempts') +
...
for (i in 1:8) {
  head_sht <- readPNG(paste0('headshots/',player_list[i],'.png'))
  tm_logo <- readPNG(paste0('logos/',team_list[i],'.png'))
  col_mid <- ifelse(i %% 2==1, 1, 1.385)
  row_mid <- .905 - (floor((i-1)/2) * .481)
  ...
  width_split_tm <- ((ncol(tm_logo)/nrow(tm_logo)) * .045)/2.8
  rasterImage(tm_logo, col_mid - width_split_tm - .3, row_mid, col_mid + width_split_tm - .3, row_mid+.045)
}
dev.off()
