...
air_yards$PlayIden <- paste0(air_yards$GAMEID,'_',air_yards$QTR,'_',ifelse(substr(air_yards$START,1,1)==1 | nchar(air_yards$START)==4,air_yards$START,substr(air_yards$START,2,nchar(air_yards$START))))
...
write.csv(air_yards, 'air_yards_updated.csv', row.names=F)
